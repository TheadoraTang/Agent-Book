...
class WeatherAgent(BaseSubAgent):
    ...
    def run(self, task: dict[str, object], context: dict[str, object]) -> SubAgentResult:
        ...
        try:
            ...
            return SubAgentResult(self.name, self.role, task, False, f"天气 API 调用失败：{error}")
...
class WebSearchAgent(BaseSubAgent):
    ...
    def run(self, task: dict[str, object], context: dict[str, object]) -> SubAgentResult:
        ...
        try:
            ...
            return SubAgentResult(self.name, self.role, task, False, f"web_search API 请求失败，状态码：{error.code}\n{detail}")
        except Exception as error:
            return SubAgentResult(self.name, self.role, task, False, f"web_search 调用失败：{error}")
...
"""
        ...
        try:
            ...
            logger.warning("Error: Decompose 失败：%s", error)
    ...
    def run_sub_tasks(
        ...
        observations = []
        context = {"user_input": user_input, "observations": previous_observations or observations}
        for task in tasks:
            agent_name = str(task.get("agent", ""))
            agent = self.sub_agents.get(agent_name)
            if not agent:
                observations.append(
                    SubAgentResult(agent_name or "UnknownAgent", "未注册副 Agent", task, False, "没有找到对应副 Agent。").to_dict()
                )
                continue
            ...
            if agent_name == "ItineraryAgent":
                task = dict(task)
                task["previous_observations"] = json.loads(json.dumps(previous_observations or observations, ensure_ascii=False))
            observation = agent.run(task, context).to_dict()
            observations.append(observation)
            context["observations"] = (previous_observations or []) + observations
        return observations
...
"""
        ...
        try:
            ...
            logger.warning("Error: Observe 决策失败：%s", error)
            return []
