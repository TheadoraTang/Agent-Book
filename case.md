...
`if` 语句是最基本的条件语句，用于根据条件的真假来执行相应的代码块。如果条件为 `True`，则执行 `if` 语句块中的代码；如果条件为 `False`，则跳过该代码块。可以通过 `input` 函数接收用户输入的年龄，然后判断是否成年。
...
`if-else` 语句用于在条件为 `True` 时执行一个代码块，在条件为 `False` 时执行另一个代码块。同样使用 `input` 函数获取用户输入的年龄进行判断。
...
当需要检查多个条件时，可以使用 `if-elif-else` 语句。`elif` 是 `else if` 的缩写，用于在前面的条件不满足时检查另一个条件。通过 `input` 函数接收用户输入的成绩，然后判断成绩等级。
...
理解分支结构最好的方式，就是**亲眼看到程序是如何执行的**。下面的交互式流程图会动态展示程序的实际执行路径：

```sbs-iframe
src: assets/branch-flow.html
title: 分支路径点亮图
height: 350px
```
...
- 输入变量 `x` 和 `y` 的值
- 点击"运行并点亮路径"按钮
- **只有实际会走的那条路径会被高亮点亮**，包括：
  - 执行的节点（条件判断和代码块）
  - 走过的路径（连接线）
  - 对应的代码行

这不是在"理解 if"，而是在"看程序真的走了一遍"。通过改变 `x` 和 `y` 的值，你可以直观地看到程序如何根据不同的条件选择不同的执行路径。

## 循环语句

### for 循环

`for` 循环用于遍历可迭代对象（如列表、元组、字符串等）中的每个元素。可以让用户输入多个水果名称，然后使用 `for` 循环遍历输出。

<!-- sbs-code -->

```python
fruit_count = int(input("请输入你要输入的水果数量："))
fruits = []
for i in range(fruit_count):
    fruit = input(f"请输入第 {i + 1} 个水果的名称：")
    fruits.append(fruit)
...
`range()` 函数常用于生成一个整数序列，结合 `for` 循环可以实现指定次数的循环。可以让用户输入循环次数，然后使用 `for` 循环输出相应次数的信息。
...
`while` 循环会在条件为 `True` 时重复执行代码块。可以让用户输入一个数字，然后通过 `while` 循环从 1 开始计数到该数字。
...
`break` 语句用于跳出当前所在的循环。可以让用户输入一系列数字，当输入 0 时，使用 `break` 语句跳出循环。

<!-- sbs-code -->

```python
while True:
    num = int(input("请输入一个数字（输入 0 退出）："))
    if num == 0:
        break
    print(f"你输入的数字是 {num}。")
```
...
`continue` 语句用于跳过当前循环的剩余部分，直接开始下一次循环。可以让用户输入一系列数字，当输入的数字为偶数时，使用 `continue` 语句跳过输出。

<!-- sbs-code -->

```python
count = int(input("请输入你要输入的数字数量："))
for i in range(count):
...
![tree-graph](assets/tree-graph.png)
...
* 一个节点 X 和它所有后代节点、以及这些节点之间的边，也组成一棵树，叫做原树的一棵（以 X 为根的）“子树（*sub-tree*）”；
* 根节点没有父节点也没有祖先节点；
...
如果你不记得“递归（*recursion*，*recursive*）”是怎么回事了，可以温习[递归函数](5vsYz6PB1uB)一章。
