...
上一章中，TravelPlanAgent 已经能够使用工具了。

但是 v0.5 里的工具调用仍然比较“程序化”：我们通常会先用关键词判断用户问题，再决定调用哪个工具。
...
用户问题里有“天气”，就调用天气工具。
用户问题里有“攻略”，就调用搜索工具。
用户问题里有“提醒”，就调用本地小贴士工具。
```

这种方式清晰、稳定，适合入门。但它还不是 Agent 最核心的形态。

真正的 Agent 不只是“被程序规则安排去调用工具”，而是会经历一个循环：

```text
Think：判断当前缺少什么信息，应该调用什么工具
Act：执行工具调用
Observe：读取工具返回结果
Think Again：判断是否还需要继续调用工具
Final：生成最终回答
```

这一章，TravelPlanAgent 会升级到 v0.6：
...
这一章我们把 TravelPlanAgent 升级到了 v0.6。

它不再只是按固定规则调用工具，而是具备了 Agent 的核心循环：
...
- 工具结果必须回填到上下文；
- Agent 循环需要设置最大轮数；
- logging 可以把运行状态和正式回答分开。

下一章，我们会继续增强 TravelPlanAgent：让它不仅能查询实时信息，还能检索自己的旅行知识库。
