...
judgeTests: |
  agent = TravelPlanAgent(llm_client=object(), system_prompt="测试系统提示词")
  user_input = "我想去杭州玩两天"
  result = agent.build_messages(user_input)
  expected = [
      {"role": "system", "content": "测试系统提示词"},
      {"role": "user", "content": agent.build_user_prompt(user_input)},
  ]

  check("返回 System 和 User 消息", result, expected)
```
