# v0.7 练习：完成 RAG Top-K 检索

```sbs-exercise
title: v0.7 练习：完成 RAG Top-K 检索
kind: completion
language: python
judgeMode: testCases
description: |
  补全 RagKnowledgeTool.run()，完成查询向量化、相似度排序和 Top-K 知识片段返回。
code: |
...
judgeTests: |
  tool = RagKnowledgeTool(knowledge_text="片段甲\n\n片段乙\n\n片段丙")
  tool.chunks = ["片段甲", "片段乙", "片段丙"]
  tool.embeddings = np.asarray(
      [
          [0.2, 0.0],
          [0.9, 0.0],
          [0.5, 0.0],
  ...
  tool.embed_texts = lambda texts: np.asarray([[1.0, 0.0]], dtype=np.float32)

  result = tool.run({"query": "哪个片段最相关？", "top_k": 2})

  check_true("返回 ToolResult", isinstance(result, ToolResult))
  if isinstance(result, ToolResult):
      check("返回两个 Top-K 结果", len(result.raw_data), 2)
      check("按相似度排序知识片段", [item["content"] for item in result.raw_data], ["片段乙", "片段丙"])
```
