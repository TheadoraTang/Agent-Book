# v0.5 练习：根据用户问题选择 Tools

```sbs-exercise
title: v0.5 练习：根据用户问题选择 Tools
kind: completion
language: python
judgeMode: testCases
description: |
  补全 select_tools()，根据天气、攻略、本地提醒等关键词选择应该调用的工具。
code: |
...
judgeTests: |
  agent = TravelPlanAgent(llm_client=object())

  check("天气问题选择天气工具", agent.select_tools("杭州明天天气怎么样？"), ["weather_api"])
  check("攻略问题选择搜索工具", agent.select_tools("帮我找杭州旅行攻略"), ["web_search"])
  check("无明确关键词时选择本地工具", agent.select_tools("我想去杭州走走"), ["local_function"])
```
