...
judgeTests: |
  ...
  result = agent.parse_agent_output(json.dumps(data, ensure_ascii=False))

  def rejects_missing_field():
      incomplete = dict(data)
      incomplete.pop("reply")
      try:
          agent.parse_agent_output(json.dumps(incomplete, ensure_ascii=False))
          return False
      except ValueError:
          return True

  check("返回解析后的结构", result, {"valid": True, "data": data})
  check_true("拒绝缺少必需字段的输出", rejects_missing_field())
```
