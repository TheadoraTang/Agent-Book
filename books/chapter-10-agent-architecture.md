...
到第 9 章，TravelPlanAgent 已经具备了几类能力：

- 调用 LLM；
- 维护上下文；
- 调用本地函数、天气 API、搜索工具；
- 通过 RAG 检索旅游知识；
- 使用 Think-Act-Observe 循环自主选择工具。
...
如果所有事情都塞进一个函数，代码会越来越乱。

第 10 章会让 TravelPlanAgent 升级到 v0.8：

```text
具备完整旅行规划能力，能够拆解任务，并调度多个子 Agent 协作
```
...
它的核心不是“Agent 越多越高级”，而是通过角色拆分降低复杂度。主 Agent 负责理解目标、拆解任务、调度子 Agent、汇总结果；子 Agent 负责完成某一类稳定职责。
...
它解决的问题不是“如何执行更多工具”，而是“如何让输出更可靠”。

很多 Agent 的失败并不是因为没有工具，而是因为没有检查：
...
RAG 的目标不是把知识库整段搬给模型，而是只提供最相关的依据。
...
这一章我们把 TravelPlanAgent 升级到了 v0.8。

它已经不只是会调用工具，而是具备更完整的旅行规划架构：
...
- Plan & Execute 适合复杂任务；
- Multi-Agent 可以让职责更清晰；
- Reflection 可以让 Agent 检查是否缺少信息；
- `SubAgentResult` 统一了子 Agent 输出；
- 最终回答应服务用户，而不是暴露内部流程；
- v0.8 已经是一个具备完整旅行规划能力的 Agent。

下一部分，我们会进入成熟 Agent 框架，看看 Pydantic AI 等框架如何把这些结构进一步标准化。
