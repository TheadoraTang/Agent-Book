...
judgeTests: |
  class FakeCompletions:
      ...
      def create(self, **kwargs):
          ...
          message = SimpleNamespace(content="测试模型回复")
  ...
  client = LLMClient(openai_client=fake_openai)
  messages = [{"role": "user", "content": "测试消息"}]
  result = client.chat(messages)

  check("返回模型回复", result, "测试模型回复")
  check_true("调用模型客户端", fake_completions.received is not None)
  if fake_completions.received is not None:
      check("把 messages 传给模型", fake_completions.received.get("messages"), messages)
```
<!--sbs-code-->
```python
from openai import OpenAI
