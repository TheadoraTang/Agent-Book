...
- **API 调用**：程序按约定向模型服务发请求，再按约定取回结果。
- **base_url**：模型服务的地址，像寄信时要写清楚收件地点。
- **api_key**：调用模型服务的钥匙，用来证明这次请求有权限。
- **messages / role**：把一句句对话按身份装好，再交给模型理解。

### 产品功能增量

- 本章要跑通 TravelPlanAgent v0.1 的最小链路：命令行输入一句话，程序调用 LLM，再把回复显示出来。


## 对 TravelPlanAgent 来说意味着什么

上一章我们知道了：LLM 会根据上下文生成内容。现在要往前走一步，让自己的程序真的能把上下文发给模型。

TravelPlanAgent v0.1 的目标很小：

```text
能够调用 LLM 进行简单的命令行聊天
```

为了做到这件事，我们只需要打通一条最短的路：

- 用户在命令行输入一句话；
- 程序把它整理成 `messages`；
- 程序带着 `base_url`、`api_key`、`model_name` 去请求模型；
- 模型返回回答；
- 程序把回答打印出来。

这个版本还不会记住历史，也不会查天气，更不会规划路线。没关系，先把这条链路跑通就够了。后面所有上下文、工具、RAG、任务规划，都是在这条链路旁边慢慢长出来的。

如果你以前只在网页聊天框里用过大模型，这一章要解决的就是一个很朴素的问题：聊天框背后那次“发送”和“返回”，放到自己的代码里应该怎么写？
...
工具调用、RAG、多轮记忆、任务规划，后面都会沿着这条基础链路继续加结构。
...
在课程中，地址由平台自行提供。学习时不需要死记某个地址，重点是理解它的作用：告诉程序去哪里请求模型服务。
...
在教材代码中，我们为你提供了内置的API_KEY，但你在实际运行时应该替换成从安全位置读取的密钥（比如环境变量或者是更安全的用动态注入）
...
在 Agent 开发中，选模型先看任务，不必一上来就追求“最强”：
...
在本课程中，我们提供的是千问大模型，你在自己本地学习和使用的时候可以自行选择合适的模型，接下来我们会推荐给你们一些可用的API。

### 常见 API 推荐

刚开始学习时，最容易卡住的不是“模型原理”，而是：我到底该去哪里申请一个能用的 API？

下面这些都是比较常见、面向大众开发者开放的大模型 API。它们只是学习和课程项目里比较容易上手的参考清单，不代表唯一选择或性能排名。实际使用前，请以各平台控制台和官方文档中的最新模型名、价格、额度和地区限制为准。
...
这些字段不是装饰，它们会告诉模型：这句话从哪里来，应该按什么身份理解。
...
到这里，“网页里聊天”背后的动作已经被拆成了一次程序可以执行的 API 调用。

以后看到一段 LLM 调用代码，你可以先找这几个位置：

- `base_url`：请求发到哪里；
- `api_key`：这次调用有没有权限；
- `model_name`：要用哪个模型；
- `messages`：模型这次能看见什么；
- `system`、`user`、`assistant`：每句话分别是谁说的。

到这里，TravelPlanAgent v0.1 的地基就有了。下一步进入代码实践时，我们会把这些参数真正写进程序，让命令行里的用户输入能发给模型。
