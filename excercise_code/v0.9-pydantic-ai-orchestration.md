# v0.9 练习：串起框架版 Think-Act-Observe

这道题基于当前 `Agent-Code/TravelPlanAgent_v0.9.py` 改写。代码保留完整 Agent 结构，只挖去当前版本最核心的一小段实现。

```sbs-exercise
title: v0.9 练习：串起框架版 Think-Act-Observe
kind: completion
language: python
...
class LLMConfig:
    def __init__(
        ...
        self.base_url = base_url or os.getenv("TRAVEL_AGENT_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
        self.api_key = api_key or os.getenv("TRAVEL_AGENT_API_KEY", "")
        self.model_name = model_name or os.getenv("TRAVEL_AGENT_MODEL", "deepseek-v4-flash")
